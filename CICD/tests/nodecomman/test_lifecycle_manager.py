...
class TestProcessLifecycleManager:
    ...
    @pytest.mark.asyncio
    async def test_register_process(
        self, lifecycle_manager, mock_process, lifecycle_config, server_config
    ):
        """測試進程註冊"""
    ...
    async def test_restart_on_failure(
        self, lifecycle_manager, lifecycle_config, server_config
    ):
        """測試失敗時自動重啟"""
        process_id = "test_process"
        ...
        with patch.object(
            lifecycle_manager, "_create_new_process", side_effect=mock_restart_process
        ):
            await lifecycle_manager.start_process(process_id)
    ...
    @pytest.mark.asyncio
    async def test_process_metrics_collection(
        self, lifecycle_manager, mock_process, lifecycle_config, server_config
    ):
        """測試進程指標收集"""
        process_id = "test_process"
...
class TestRestartPolicies:
    ...
    async def test_never_restart_policy(self, lifecycle_manager):
        ...
        await lifecycle_manager.register_process("test", failing_process, config, None)
    ...
    async def test_restart_attempts_limit(self, lifecycle_manager):
        ...
        config = LifecycleConfig(
            restart_policy=RestartPolicy.ON_FAILURE, max_restart_attempts=2
        )
