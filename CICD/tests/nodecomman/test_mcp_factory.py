...
class TestConfigurationAPIConsistency:
    ...
    def test_config_class_compatibility(self, mcp_factory, base_config_manager):
        ...
        if postgres_base_config:
            ...
            assert (
                isinstance(postgres_base_config.command, str)
                or postgres_base_config.command is None
            )
    ...
    def test_mcp_config_manager_api_consistency(self, base_config_manager):
        ...
        for method_name in required_methods:
            ...
            assert callable(
                getattr(base_config_manager, method_name)
            ), f"方法不可調用: {method_name}"
...
class TestServiceRegistryIntegration:
    ...
    async def test_enhanced_service_factory_integration(self):
        ...
        try:
            ...
            nodecomman_methods = ["get_enhanced_mcp_client", "get_enhanced_mcp_config"]
    ...
    async def test_enhanced_mcp_client_parameters(self):
        ...
        try:
            ...
            # 測試不同參數組合
            client1 = get_enhanced_mcp_client(
                use_nodecomman=True, fallback_enabled=True
            )
            assert client1 is not None

            client2 = get_enhanced_mcp_client(
