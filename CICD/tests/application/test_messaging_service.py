...
class TestMessagingApplicationService:
    ...
    def mock_command_executor(self):
        ...
        executor.execute_command = AsyncMock(
            return_value=TextMessage(text="test response")
        )
        executor.initialize = Mock()
        executor.get_command_info = Mock(
            return_value={"total_commands": 5, "total_aliases": 3}
        )
    ...
    async def test_process_command_message(self, messaging_service):
        ...
        with patch("src.utils.observability.get_tracer"):
            ...
            messaging_service._command_executor.parse_and_validate_command.return_value = (
                "help",
                [],
            )
            ...
            messaging_service._command_executor.execute_command.assert_called_once_with(
                "test_user", "/help"
            )
