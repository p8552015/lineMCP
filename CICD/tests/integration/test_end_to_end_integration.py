...
class TestEndToEndIntegration:
    ...
    async def test_machine_query_flow(self, application_facade):
        ...
        try:
            ...
            # 回應應該包含機台相關資訊
            assert any(
                keyword in response.lower() for keyword in ["機台", "m001", "狀態"]
            )
    ...
    async def test_fault_query_flow(self, application_facade):
        ...
        try:
            ...
            # 回應應該包含故障相關資訊
            assert any(
                keyword in response.lower() for keyword in ["故障", "問題", "維修"]
            )
    ...
    async def test_utilization_query_flow(self, application_facade):
        ...
        try:
            ...
            # 回應應該包含使用率相關資訊
            assert any(
                keyword in response.lower() for keyword in ["使用率", "效率", "利用率"]
            )
...
class TestLineWebhookIntegration:
    ...
    async def test_webhook_message_processing(
        self, application_facade, mock_line_bot_api
    ):
        """測試 Webhook 訊息處理"""
        # 模擬 LINE Webhook 事件
        mock_event = Mock()
    ...
    async def test_sql_generation_and_execution(self, application_facade):
        ...
        for query in natural_queries:
            try:
                ...
                assert not any(
                    keyword in response.lower() for keyword in error_keywords
                )
...
class TestSystemResilience:
    ...
    async def test_query_timeout_handling(self, application_facade):
        ...
        with patch("asyncio.wait_for") as mock_wait_for:
            mock_wait_for.side_effect = TimeoutError("Query timeout")
    ...
    async def test_concurrent_request_handling(self, application_facade):
        ...
        # 至少一半的查詢應該成功
        successful_results = [
            r for r in results if isinstance(r, str) and "查詢失敗" not in r
        ]
