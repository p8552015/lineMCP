...
try:
    from infrastructure.service_registry import get_service_registry, ServiceScope
...
class TestServiceRegistryValidation:
    ...
    def test_service_factory_initialization(self):
        ...
        try:
            ...
            assert info["total_services"] >= expected_min_services, f"期望至少 {expected_min_services} 個服務，實際 {info['total_services']} 個"
    ...
    def test_all_core_services_registered(self):
        ...
        try:
            ...
            for service_name in critical_services:
                try:
                    ...
                    if hasattr(provider, service_method):
                        ...
                        # 檢查是否在註冊表中
                        registry_services = [str(service_type) for service_type in registry._services.keys()]
                        if any(service_name.lower() in reg_service.lower() for reg_service in registry_services):
                            available_services.append(service_name)
            ...
            # 記錄結果
            self.test_results["status"] = "PASS" if len(missing_services) == 0 else "PARTIAL"
            self.test_results["details"] = {
            ...
            print(f"   覆蓋率: {len(available_services) / len(critical_services) * 100:.1f}%")
            ...
            assert actual_coverage >= min_coverage, f"核心服務覆蓋率 {actual_coverage:.1f}% 低於要求的 {min_coverage}%"
    ...
    def test_service_scopes_validation(self):
        ...
        try:
            ...
            try:
                ...
                print(f"✅ Singleton 測試: {'通過' if singleton_test_passed else '失敗'}")
    ...
    def test_service_resolution_performance(self):
        ...
        try:
            ...
            self.test_results["status"] = "PASS" if avg_time <= performance_threshold else "PARTIAL"
            self.test_results["details"] = {
            ...
            print(f"   達標狀態: {'✅ 通過' if avg_time <= performance_threshold else '⚠️ 超出門檻'}")
            
            if avg_time <= performance_threshold:
                print("✅ 服務解析效能測試通過")
            else:
                print(f"⚠️ 服務解析平均時間 {avg_time:.2f}ms 超過門檻 {performance_threshold}ms")
    ...
    def test_dependency_injection_health(self):
        ...
        try:
            ...
            # 檢查服務註冊表的健康狀況
            registry = get_service_registry()
            registry_services = registry._services if hasattr(registry, '_services') else {}
            ...
            resolution_success_rate = successful_resolutions / len(test_services) * 100 if test_services else 0
...
def generate_service_registry_report(test_results_list):
    ...
    report = {
        ...
        "summary": {
            ...
            "partial": sum(1 for r in test_results_list if r.get("status") == "PARTIAL"),
            "failed": sum(1 for r in test_results_list if r.get("status") in ["FAIL", "ERROR"])
        },
