...
class TestPostgresMCP:
    ...
    def test_docker_environment_check(self):
        ...
        try:
            ...
            if result != 0:
                ...
                os.system("cd /Users/yen/Desktop/lineMCP && docker-compose -f docker-compose.postgres.yml up -d")
                time.sleep(5)  # 等待容器啟動
            ...
            self.test_results["details"]["docker_status"] = "運行中" if result == 0 else "未運行"
    ...
    def test_postgresql_direct_connection(self):
        ...
        try:
            ...
            self.test_results["details"]["postgresql_version"] = version[0] if version else "Unknown"
            
            print(f"✅ PostgreSQL 直接連接成功")
