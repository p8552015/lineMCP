...
class CIMonitorDaemon:
    ...
    async def _handle_status_change(self, report: Dict[str, Any]):
        """處理狀態變化"""
        self.logger.info(f"🔄 CI 狀態變化: {report['previous_status']} → {report['conclusion']}")
            ...
            for job in failed_jobs:
                ...
                if job["steps"]:
                    for step in job["steps"]:
                        self.logger.error(f"       * {step['name']}: {step['conclusion']}")
    ...
    async def start_monitoring(self):
        ...
        try:
            while self.running:
                ...
                # 等待下次檢查
                if self.running:
                    self.logger.debug(f"⏰ 等待 {self.check_interval} 秒後進行下次檢查...")
    ...
    def save_monitoring_report(self):
        ...
        try:
            ...
            # 保存歷史報告
            history_file = report_dir / f"monitor_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
