        ...
        try:
            ...
            if timeout:
                stdout, stderr = await asyncio.wait_for(
                    self._process.communicate(input_bytes),
                    timeout=timeout
                )
            else:
...
class NodeJSRuntimeManager(IRuntimeManager):
    ...
    async def get_runtime_info(self) -> RuntimeInfo:
        ...
        try:
            ...
            node_version = node_version_result["stdout"].strip() if node_version_result["success"] else "未知"
            
            npm_version_result = await self._run_command(["npm", "--version"])
            npm_version = npm_version_result["stdout"].strip() if npm_version_result["success"] else "未知"
    ...
    async def validate_command(self, command: str, args: List[str]) -> bool:
        ...
        try:
            ...
            if command == "npx" and args:
                package_name = args[0] if args[0] != "-y" else (args[1] if len(args) > 1 else "")
                if package_name:
                    # 檢查套件是否已安裝或可安裝
