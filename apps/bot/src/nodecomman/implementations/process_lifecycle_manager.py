...
from ..interfaces.runtime_interfaces import IProcess, ProcessInfo
...
class LifecycleConfig:
    ...
    memory_limit_mb: Optional[int] = None  # 記憶體限制（MB）
    cpu_limit_percent: Optional[float] = None  # CPU 限制（百分比）
...
class ManagedProcess:
    ...
    # 回調函數
    on_state_change: Optional[Callable[['ManagedProcess', LifecycleState, LifecycleState], None]] = None
    on_health_check_failed: Optional[Callable[['ManagedProcess'], None]] = None
    on_restart: Optional[Callable[['ManagedProcess'], None]] = None
...
class ProcessLifecycleManager:
    ...
    async def start_process(self, process_id: str) -> bool:
        ...
        try:
            async with self._lock:
                ...
                if managed_process.state in [LifecycleState.RUNNING, LifecycleState.STARTING]:
    ...
    async def stop_process(self, process_id: str, force: bool = False) -> bool:
        ...
        try:
            async with self._lock:
                ...
                if managed_process.state in [LifecycleState.STOPPED, LifecycleState.STOPPING]:
                ...
                # 停止進程
                timeout = managed_process.config.force_kill_timeout if force else managed_process.config.graceful_shutdown_timeout
    ...
    async def restart_process(self, process_id: str) -> bool:
        ...
        try:
            ...
            # 等待重啟延遲
            delay = min(
                managed_process.config.restart_delay * (managed_process.config.backoff_multiplier ** managed_process.metrics.restart_count),
            ...
            if await self.start_process(process_id):
                ...
                logger.info(f"✅ 進程 {process_id} 重啟成功 (第{managed_process.metrics.restart_count}次)")
                return True
            else:
                managed_process.set_state(LifecycleState.FAILED)
    ...
    async def _monitor_process(self, process_id: str):
        ...
        try:
            ...
            while not self._is_shutting_down and managed_process.state == LifecycleState.RUNNING:
                try:
                    # 健康檢查
                    ...
                    if is_healthy:
                        ...
                        # 檢查是否需要重啟
                        await self._handle_unhealthy_process(process_id, managed_process)
    ...
    async def _handle_unhealthy_process(self, process_id: str, managed_process: ManagedProcess):
        """處理不健康的進程"""
        try:
