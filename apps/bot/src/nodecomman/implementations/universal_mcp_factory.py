...
class UniversalMCPServerFactory(IMCPServerFactory):
    ...
    def _load_predefined_configs(self) -> Dict[str, MCPServerConfig]:
        ...
        return {
            ...
            "sqlite": MCPServerConfig(
                ...
                runtime_type=RuntimeType.NODEJS,
                command="npx",
                args=["-y", "@modelcontextprotocol/server-sqlite", "--db-path", "./data/sqlite/test.db"],
                env={},
                working_directory=None,
                auto_restart=True,
            ...
            "filesystem": MCPServerConfig(
                ...
                runtime_type=RuntimeType.NODEJS,
                command="npx",
                args=["-y", "@modelcontextprotocol/server-filesystem", "--base-path", "/Users/yen/Desktop/lineMCP"],
                env={},
                working_directory=None,
                auto_restart=False,
                ...
                if config.runtime_type == RuntimeType.NODEJS and config.command == "npx":
                    ...
                    if package_name:
                        ...
                        if not await runtime_manager.install_dependencies([package_name]):
                            logger.warning(f"⚠️ 套件安裝失敗，但將繼續嘗試執行")
    ...
    async def start_server(self, server_name: str) -> bool:
        ...
        try:
            ...
            if await process.start():
                ...
                logger.info(f"✅ MCP 服務器 {server_name} 啟動成功 (PID: {server_info.process_id})")
                return True
            else:
    ...
    async def get_predefined_config(self, config_name: str) -> Optional[MCPServerConfig]:
        """獲取預定義配置"""
    ...
    async def create_from_predefined(self, config_name: str, **overrides) -> MCPServerInfo:
        """從預定義配置創建服務器"""
    ...
    def _extract_package_name(self, args: List[str]) -> Optional[str]:
        """從 npx 參數中提取套件名稱"""
        try:
            # 跳過 -y 等選項
            for arg in args:
    ...
    async def get_default_config(
        ...
        elif server_type == MCPServerType.FILESYSTEM and runtime_type == RuntimeType.NODEJS:
