...
from .runtime_interfaces import RuntimeType, IProcess
...
class MCPServerInfo:
    ...
    connection_info: Optional[MCPConnectionInfo] = None
    runtime_info: Optional[Any] = None
    ...
    def is_healthy(self) -> bool:
        """檢查服務器是否健康"""
        return (
            self.status == MCPServerStatus.RUNNING and
            self.process_id is not None and
            (self.last_health_check is None or 
             __import__('time').time() - self.last_health_check < 60)
        )
...
class MCPServerConfig:
    ...
    working_directory: Optional[str] = None  # 修改為一致的名稱
...
class IMCPServer(ABC):
    ...
    @property
    @abstractmethod
    def process(self) -> Optional[IProcess]:
        """獲取服務器進程（如果適用）"""
        pass
...
class IMCPServerManager(ABC):
    ...
    @abstractmethod
    async def get_server(self, name: str) -> Optional[IMCPServer]:
        """
        獲取指定名稱的服務器
