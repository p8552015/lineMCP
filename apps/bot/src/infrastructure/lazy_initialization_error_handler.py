...
class LazyInitializationErrorHandler:
    ...
    async def safe_initialize(self, initializer: Callable[[], Any]) -> tuple[bool, Any, Optional[str]]:
        """
        安全的延遲初始化
    ...
    async def _attempt_initialization(self, initializer: Callable[[], Any]) -> tuple[bool, Any, Optional[str]]:
        """嘗試初始化"""
    ...
    def _create_error_record(self, exception: Exception, retry_count: int) -> InitializationError:
        """創建錯誤記錄"""
    ...
    def _assess_error_severity(self, error_type: str, error_message: str) -> ErrorSeverity:
        ...
        # 致命錯誤
        if any(keyword in message_lower for keyword in ["out of memory", "disk full", "system", "fatal"]):
            return ErrorSeverity.CRITICAL
        
        # 高嚴重性錯誤
        if any(keyword in message_lower for keyword in ["permission denied", "access denied", "unauthorized"]):
            return ErrorSeverity.HIGH
        
        # 中等嚴重性錯誤
        if any(keyword in message_lower for keyword in ["connection", "timeout", "network", "import"]):
    ...
    def _generate_user_error_message(self, error: InitializationError) -> str:
        ...
        base_message = self.user_messages.get(error_category, self.user_messages["unknown"])
    ...
    async def _attempt_auto_recovery(self) -> bool:
        ...
        logger.info(f"🔧 嘗試自動恢復 {self.config.service_name} (第{self._recovery_attempts}次)")
...
class LazyServiceRegistry:
    ...
    def register_service(self, service_name: str, config: LazyServiceConfig) -> LazyInitializationErrorHandler:
        """註冊延遲服務"""
        handler = LazyInitializationErrorHandler(config)
    ...
    def get_handler(self, service_name: str) -> Optional[LazyInitializationErrorHandler]:
        """獲取處理器"""
