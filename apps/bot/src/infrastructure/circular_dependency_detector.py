...
class CircularDependencyDetector:
    ...
    def add_dependency(self, from_service: str, to_service: str, 
                      ...
                      stack_trace: Optional[str] = None) -> CycleDetectionReport:
        """
        添加依賴關係並檢測循環
        ...
        with self._lock:
            ...
            if cycle_report.result == CycleDetectionResult.NO_CYCLE:
                ...
                logger.debug(f"添加依賴: {from_service} -> {to_service} ({dependency_type.value})")
            else:
                logger.warning(f"檢測到循環依賴: {from_service} -> {to_service}")
                cycle_report.suggestions = self._generate_cycle_resolution_suggestions(cycle_report)
    ...
    def begin_resolution(self, service_name: str) -> bool:
        ...
        with self._lock:
            if service_name in self._resolution_stack:
                # 檢測到正在解析中的服務，可能是循環
                cycle_path = self._resolution_stack[self._resolution_stack.index(service_name):]
    ...
    def end_resolution(self, service_name: str) -> None:
        ...
        with self._lock:
            if self._resolution_stack and self._resolution_stack[-1] == service_name:
                ...
                logger.debug(f"完成解析服務: {service_name}, 剩餘深度: {len(self._resolution_stack)}")
    ...
    def _generate_cycle_resolution_suggestions(self, report: CycleDetectionReport) -> List[str]:
        """生成循環解析建議"""
        suggestions = []
        ...
        # 檢查是否有工廠類服務
        factory_services = [s for s in cycle_services if any(pattern in s.lower() for pattern in self._risky_patterns)]
        ...
        # 檢查依賴類型
        constructor_deps = [e for e in report.cycle_edges if e.dependency_type == DependencyType.CONSTRUCTOR]
        if constructor_deps:
    ...
    def get_dependency_graph_summary(self) -> Dict[str, Any]:
        ...
        with self._lock:
            return {
                ...
                "cycles_detected": len([r for r in [self._detect_cycles_in_graph(self._dependency_graph)] 
...
# 全局檢測器實例
_global_detector: Optional[CircularDependencyDetector] = None
