...
def dependency_guard(service_name: str = None, dependency_type: DependencyType = DependencyType.CONSTRUCTOR):
    """
    依賴注入防護裝飾器
        ...
        def wrapper(*args, **kwargs) -> T:
            ...
            try:
                ...
                if inspect.isclass(result):
                    ...
                    detector.register_service_type(actual_service_name, result.__class__)
...
def service_dependency(from_service: str, to_service: str, 
...
class DependencyInjectionGuard:
    ...
    def _analyze_constructor_dependencies(self, service_class: Type, args: tuple, kwargs: dict) -> None:
        """分析構造函數依賴"""
        ...
        for param_name, value in bound_args.arguments.items():
            ...
            if hasattr(value, '__class__') and hasattr(value.__class__, '__name__'):
                ...
                if any(keyword in dependency_name.lower() 
                       for keyword in ['service', 'factory', 'client', 'handler', 'manager']):
    ...
    def check_system_health(self) -> dict[str, Any]:
        ...
        if risk_analysis["high_risk_services"]:
            ...
            health_status["issues"].append(f"發現 {len(risk_analysis['high_risk_services'])} 個高風險服務")
        
        if graph_summary["cycles_detected"] > 0:
            health_status["risk_level"] = "critical"
            health_status["status"] = "error"
            health_status["issues"].append(f"檢測到 {graph_summary['cycles_detected']} 個循環依賴")
