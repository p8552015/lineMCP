...
from .unified_mcp_client import get_unified_mcp_client
...
class MessageHandler:
    ...
    def __init__(self):
        ...
        self.nl_service = NaturalLanguageToSQLService(self.ai_model_service)
        self.db_service = DatabaseService(self._get_mcp_client)
        self.formatter = MessageFormatter()

        # TaskMaster 整合暫時禁用
        self.taskmaster = None

        logger.info("✅ 重構版訊息處理器初始化完成")
        logger.info("🤖 AI模型服務：已啟用")
        logger.info("🧠 自然語言轉SQL服務：已啟用")
        logger.info("🗄️ 資料庫服務：已啟用")
        logger.info("📝 訊息格式化服務：已啟用")

    async def _get_mcp_client(self):
        """獲取 MCP 客戶端（延遲初始化）"""
        if self._mcp_client is None:
            self._mcp_client = await get_unified_mcp_client()
    ...
    async def process_message(
        ...
        with tracer.start_as_current_span("process_message") as span:
            ...
            try:
                ...
                return TextMessage(text="💥 訊息處理時發生錯誤，請稍後再試")
    ...
    async def _handle_command(self, user_id: str, command: Command) -> Message:
        ...
        if handler:
            return await handler(user_id, command.args)
        else:
            return TextMessage(
                text=f"❌ 未知的指令: {command.name}\\n💡 使用 /help 查看可用指令"
            )
