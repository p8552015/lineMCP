...
class QueryStatisticsService(IStatistics):
    ...
    def record_success(
        ...
        try:
            ...
            parser_type = (
                metadata.get("parser_type", operation_type)
                if metadata
                else operation_type
            )
            confidence = self._safe_confidence_conversion(
                metadata.get("confidence", 1.0) if metadata else 1.0
            )
    ...
    def get_stats(self) -> dict[str, Any]:
        ...
        # 計算成功率 - 類型安全的除法
        success_rate = (
            float(total_success) / float(total_requests) if total_requests > 0 else 0.0
        )
        ...
        avg_parse_time = (
            sum(all_parse_times) / len(all_parse_times) if all_parse_times else 0.0
        )

        # 解析器效能統計 - 類型安全版本
        parser_stats = {}
        for parser_type in set(
            ...
            # 確保統計數據的類型安全
            confidences = self._safe_numeric_list_conversion(
                self._confidence_scores[parser_type]
            )
    ...
    def get_parser_performance(self, parser_type: str) -> dict[str, Any]:
        ...
        return {
            "parser_type": parser_type,
            "request_counts": {"success": success, "failure": failure, "total": total},
            "success_rate": float(success) / float(total) if total > 0 else 0.0,
            "confidence_metrics": {
    ...
    def _cleanup_old_data(self, parser_type: str, max_records: int = 1000) -> None:
        ...
        if len(self._confidence_scores[parser_type]) > max_records:
            self._confidence_scores[parser_type] = self._confidence_scores[parser_type][
                -max_records:
            ]

        # 清理時間記錄
        if len(self._parse_times[parser_type]) > max_records:
            self._parse_times[parser_type] = self._parse_times[parser_type][
                -max_records:
            ]
    ...
    def _grade_performance(self, parse_times: list[float]) -> str:
        ...
        try:
            ...
            logger.warning(
                "平均時間計算失敗", parse_times_count=len(numeric_times), error=str(e)
            )
            return "unknown"
    ...
    def _analyze_trend_direction(self, recent_records: list[dict[str, Any]]) -> str:
        ...
        try:
            first_success_rate = (
                float(first_success_count) / float(first_half_len)
                if first_half_len > 0
                else 0.0
            )
            second_success_rate = (
                float(second_success_count) / float(second_half_len)
    ...
    def _export_to_csv(self, stats: dict[str, Any]) -> str:
        ...
        # 標題行
        lines.append(
            "Parser,Success,Failure,Success_Rate,Avg_Confidence,Avg_Parse_Time"
        )

        # 資料行
        for parser, data in stats["parser_statistics"].items():
            lines.append(
                f"{parser},{data['success_count']},{data['failure_count']},"
    ...
    def _safe_confidence_conversion(
        self, confidence: Any, default: float = 1.0
    ) -> float:
        """
    ...
    def _validate_statistics_integrity(self) -> dict[str, Any]:
        ...
        try:
            ...
            for parser_type in all_parser_types:
                ...
                # 檢查時間記錄是否有異常值
                times = self._safe_numeric_list_conversion(
                    self._parse_times[parser_type]
                )
                if times:
    ...
    def _get_detailed_conversion_diagnostic(
    ...
    def get_service_health(self) -> dict[str, Any]:
        ...
        try:
            ...
            health_status = {
                ...
                "total_requests": total_requests,
                "success_rate": (
                    float(total_success) / float(total_requests)
                    if total_requests > 0
                    else 0.0
                ),
                ...
                "memory_usage": {
                    ...
                    "usage_percentage": (len(self._history) / self._max_history_size)
                    * 100,
                },
                "active_parsers": len(
                    set(
                ...
                "last_activity": (
                    max([r.get("timestamp", 0) for r in self._history])
                    if self._history
    ...
    def record_event(
        ...
        try:
            ...
            self._history.append(
                {
                    ...
                    "result": (
                        ...
                        else ("failure" if "failure" in event_name.lower() else "info")
                    ),
            ...
            if "success" in event_name.lower():
                parser_type = (
                    metadata.get("parser_type", "unknown") if metadata else "unknown"
                )
                self._success_counts[parser_type] += 1
        ...
        return {
            "parser_statistics": {
                parser: {
                    ...
                    "confidence_scores": self._confidence_scores[parser][
                        -100:
                    ],  # 最近100個
