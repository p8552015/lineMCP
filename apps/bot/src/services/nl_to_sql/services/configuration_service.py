...
class ConfigurationService(IConfiguration):
    ...
    def get_configuration_summary(self) -> Dict[str, Any]:
        ...
        if "parser_weights" in parser_settings.get("strategy_settings", {}):
            summary["parser_count"] = len(parser_settings["strategy_settings"]["parser_weights"])
    ...
    def _load_environment_variables(self) -> Dict[str, Any]:
        ...
        # 組合解析器設定
        fallback_threshold = float(os.getenv("COMPOSITE_PARSER_FALLBACK_THRESHOLD", "0.5"))
        env_config["composite_parser_fallback_threshold"] = fallback_threshold
        
        # 統計功能開關
        enable_statistics = os.getenv("ENABLE_QUERY_STATISTICS", "true").lower() == "true"
    ...
    def _merge_environment_config(self) -> None:
        ...
        env_section = {
            ...
            "feature_flags": {
                ...
                "enable_config_hot_reload": self._env_config["enable_config_hot_reload"]
            },
            "performance_settings": {
                ...
                "composite_parser_fallback_threshold": self._env_config["composite_parser_fallback_threshold"]
            }
        }
        ...
        self._config_cache["parser_settings"]["environment_overrides"].update(env_section)
    ...
    def is_feature_enabled(self, feature_name: str) -> bool:
        ...
        feature_map = {
            ...
            "config_hot_reload": self._env_config.get("enable_config_hot_reload", False)
        }
    ...
    def get_performance_setting(self, setting_name: str, default: Any = None) -> Any:
        ...
        performance_map = {
            ...
            "rule_parser_cache_size": self._env_config.get("rule_parser_cache_size", 1000),
            "composite_parser_fallback_threshold": self._env_config.get("composite_parser_fallback_threshold", 0.5)
        }
    ...
    def get_config_metadata(self) -> Dict[str, Any]:
        ...
        metadata = {
            ...
            "yaml_files_loaded": len([
                name for name, data in self._config_cache.items() 
            ...
            "features": {
                ...
                "config_hot_reload_enabled": self.is_feature_enabled("config_hot_reload")
            },
            "performance_settings": {
                "ai_parser_timeout_ms": self.get_performance_setting("ai_parser_timeout"),
                "rule_parser_cache_size": self.get_performance_setting("rule_parser_cache_size"),
                "composite_parser_fallback_threshold": self.get_performance_setting("composite_parser_fallback_threshold")
            }
        }
            ...
            try:
                if config_path.exists():
                    with open(config_path, 'r', encoding='utf-8') as file:
                        ...
                        logger.debug("📁 配置檔案載入成功",
                                   config=config_name,
                                   path=str(config_path))
                else:
                    # 降級為 debug 級別，因為會載入空配置作為後備
                    logger.debug("⚠️ 配置檔案不存在",
                                 config=config_name,
                                 path=str(config_path))
                    self._config_cache[config_name] = {}
                    
            except Exception as e:
                logger.error("❌ 配置檔案載入失敗",
                           config=config_name,
                           path=str(config_path),
                           error=str(e))
                self._config_cache[config_name] = {}
