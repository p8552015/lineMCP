...
class MCPConnectionPool:
    ...
    async def get_connection(self, server_name: str) -> Optional[ConnectionInfo]:
        ...
        async with self.connection_lock:
            ...
            # 檢查連接狀態
            if connection.status == ConnectionStatus.CONNECTED and connection.metrics.is_healthy:
                return connection
            
            # 需要建立或恢復連接
            if connection.status in [ConnectionStatus.DISCONNECTED, ConnectionStatus.FAILED]:
                await self._establish_connection(connection)
            
            return connection if connection.status == ConnectionStatus.CONNECTED else None
    ...
    async def get_pool_status(self) -> Dict[str, Any]:
        ...
        async with self.connection_lock:
            pool_status = {
                ...
                "healthy_connections": sum(1 for conn in self.connections.values() if conn.metrics.is_healthy),
                "connections": {}
            }
...
# 全域連接池實例
_global_connection_pool: Optional[MCPConnectionPool] = None
