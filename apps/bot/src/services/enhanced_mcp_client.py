...
class EnhancedMCPClient:
    ...
    def __init__(self, use_nodecomman: bool = True, fallback_enabled: bool = True):
        ...
        # 原有客戶端（作為回退機制）
        self._production_client: Optional[ProductionMCPClient] = None
        
        # nodecomman 組件
        self._mcp_factory: Optional[UniversalMCPServerFactory] = None
        self._lifecycle_manager: Optional[ProcessLifecycleManager] = None
    ...
    async def call_tool(
        ...
        try:
            if self.use_nodecomman and server_name in self._managed_servers:
                return await self._call_tool_nodecomman(server_name, tool_name, parameters, timeout)
            else:
                return await self._call_tool_fallback(server_name, tool_name, parameters, timeout)
                
        except Exception as e:
            logger.error(f"❌ 工具調用失敗: {e}")
            
            # 嘗試回退機制
            if self.use_nodecomman and server_name in self._managed_servers and self.fallback_enabled:
...
def get_enhanced_mcp_client(use_nodecomman: bool = True, fallback_enabled: bool = True) -> EnhancedMCPClient:
    """
    獲取增強型 MCP 客戶端單例
