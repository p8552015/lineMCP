...
class MetricsCollector:
    ...
    def record_line_message(self, message_type: str, user_type: str, duration: float, success: bool = True, error_type: str = None):
        """記錄 LINE 訊息處理指標"""
        if success:
            line_message_processing_total.labels(message_type=message_type, user_type=user_type).inc()
        else:
            line_message_processing_failures_total.labels(error_type=error_type or "unknown", message_type=message_type).inc()
        
        line_message_processing_duration_seconds.labels(message_type=message_type).observe(duration)
    
    def record_ai_request(self, provider: str, model: str, operation: str, duration: float, 
                         success: bool = True, error_type: str = None, input_tokens: int = 0, output_tokens: int = 0):
        """記錄 AI 模型請求指標"""
        if success:
            ai_model_requests_total.labels(provider=provider, model=model, operation=operation).inc()
            
            # 記錄 token 使用量
            if input_tokens > 0:
                ai_model_tokens_total.labels(provider=provider, model=model, type="input").inc(input_tokens)
            if output_tokens > 0:
                ai_model_tokens_total.labels(provider=provider, model=model, type="output").inc(output_tokens)
        else:
            ai_model_request_failures_total.labels(provider=provider, model=model, error_type=error_type or "unknown").inc()
        
        ai_model_request_duration_seconds.labels(provider=provider, model=model).observe(duration)
    
    def record_mcp_query(self, query_type: str, database: str, duration: float, success: bool = True, error_type: str = None):
        """記錄 MCP 查詢指標"""
        if success:
            mcp_queries_total.labels(query_type=query_type, database=database).inc()
        else:
            mcp_query_failures_total.labels(error_type=error_type or "unknown", database=database).inc()
    ...
    def decorator(func):
        ...
        async def wrapper(*args, **kwargs):
            ...
            try:
                ...
                metrics_collector.record_http_request(method_name, endpoint_name, status_code, time.time() - start_time)
                return result
            except Exception as e:
                metrics_collector.record_http_request(method_name, endpoint_name, 500, time.time() - start_time)
                raise
        return wrapper
    return decorator


def monitor_line_message_processing(message_type: str = "unknown", user_type: str = "unknown"):
    """LINE 訊息處理監控裝飾器"""
    def decorator(func):
        @wraps(func)
        async def wrapper(*args, **kwargs):
            ...
            try:
                result = await func(*args, **kwargs)
                metrics_collector.record_line_message(message_type, user_type, time.time() - start_time, True)
                return result
            except Exception as e:
                error_type = type(e).__name__
                metrics_collector.record_line_message(message_type, user_type, time.time() - start_time, False, error_type)
                raise
        return wrapper
    return decorator
    ...
    def decorator(func):
        ...
        async def wrapper(*args, **kwargs):
            ...
            try:
                ...
                metrics_collector.record_ai_request(provider, model, operation, time.time() - start_time, 
                                                   True, None, input_tokens, output_tokens)
                return result
            except Exception as e:
                error_type = type(e).__name__
                metrics_collector.record_ai_request(provider, model, operation, time.time() - start_time, False, error_type)
                raise
        return wrapper
    return decorator
...
def monitor_mcp_queries(query_type: str = "select", database: str = "default"):
    ...
    def decorator(func):
        ...
        async def wrapper(*args, **kwargs):
            ...
            try:
                result = await func(*args, **kwargs)
                metrics_collector.record_mcp_query(query_type, database, time.time() - start_time, True)
                return result
            except Exception as e:
                error_type = type(e).__name__
                metrics_collector.record_mcp_query(query_type, database, time.time() - start_time, False, error_type)
                raise
        return wrapper
    return decorator
