...
# 整合 nodecomman 架構
try:
    from ..nodecomman.implementations.universal_mcp_factory import UniversalMCPServerFactory
    ...
    from ..nodecomman.interfaces.server_interfaces import MCPServerConfig as NodecommanServerConfig
...
class EnhancedMCPConfig:
    ...
    def __init__(self, enable_nodecomman: bool = True):
        ...
        # nodecomman 組件
        self._mcp_factory: Optional[UniversalMCPServerFactory] = None
        self._nodejs_runtime: Optional[NodeJSRuntimeManager] = None
        self._python_runtime: Optional[PythonRuntimeManager] = None
    ...
    async def _analyze_nodejs_environment(self) -> Optional[RuntimeEnvironmentInfo]:
        """分析 Node.js 環境"""
        try:
            ...
            return RuntimeEnvironmentInfo(
                ...
                package_manager="npm" if "npm" in runtime_info.additional_info else None,
                issues=issues,
    ...
    async def _analyze_python_environment(self) -> Optional[RuntimeEnvironmentInfo]:
        """分析 Python 環境"""
        try:
    ...
    async def analyze_server_config(self, server_name: str) -> Optional[ServerConfigAnalysis]:
        """分析特定服務器配置"""
        try:
            # 獲取當前配置
    ...
    async def _enhance_config_analysis(self, analysis: ServerConfigAnalysis):
        ...
        try:
            ...
            nodecomman_config = await self._mcp_factory.get_predefined_config(server_name)
            if nodecomman_config:
                ...
                # 驗證配置
                validation_issues = await self._mcp_factory.validate_config(nodecomman_config)
                ...
                # 生成優化建議
                await self._generate_optimization_suggestions(analysis, nodecomman_config)
            else:
    ...
    async def get_comprehensive_report(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "recommendations": self._generate_system_recommendations(environments, servers_analysis)
            }
    ...
    async def get_optimal_config_for_server(self, server_name: str) -> Optional[Dict[str, Any]]:
        """獲取服務器的最佳配置建議"""
        try:
            analysis = await self.analyze_server_config(server_name)
...
# 單例管理
_enhanced_mcp_config: Optional[EnhancedMCPConfig] = None
