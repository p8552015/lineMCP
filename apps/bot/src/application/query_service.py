...
class QueryApplicationService(BaseApplicationService):
    ...
    def _validate_query(self, query: str) -> None:
        ...
        if not any(query_lower.startswith(prefix) for prefix in allowed_prefixes):
            raise create_validation_error(
                "query", 
                query, 
                "只允許 SELECT、PRAGMA 和 EXPLAIN 查詢"
            )
    ...
    def _update_query_stats(self, execution_time: float, success: bool) -> None:
        ...
        # 更新平均執行時間
        total_time = (self._query_stats["avg_execution_time"] * 
                     (self._query_stats["total_queries"] - 1) + execution_time)
        self._query_stats["avg_execution_time"] = total_time / self._query_stats["total_queries"]
    ...
    def get_query_statistics(self) -> Dict[str, Any]:
        ...
        return {
            **self._query_stats,
            ...
            "success_rate": (
                (self._query_stats["total_queries"] - self._query_stats["failed_queries"]) /
                max(self._query_stats["total_queries"], 1)
            ) * 100,
