...
from .base_service import ApplicationServiceContext
        ...
        try:
            ...
            if self._monitoring_service:
                self._monitoring_service.record_request_metrics(
                    duration=duration, status="success", endpoint="process_message"
                )
            ...
            if self._monitoring_service:
                self._monitoring_service.record_request_metrics(
                    duration=duration, status="error", endpoint="process_message"
                )
            raise

    async def execute_sql_query(
        self, query: str, user_id: str, use_cache: bool = True
    ) -> dict[str, Any]:
        """
        執行 SQL 查詢（高層次介面）
        ...
        try:
            ...
            if self._monitoring_service:
                self._monitoring_service.record_request_metrics(
                    duration=duration, status="success", endpoint="execute_sql_query"
                )

                # 記錄查詢特定指標
                self._monitoring_service.record_metric(
                    "query.rows_returned", result.get("row_count", 0), "count"
                )
            ...
            if self._monitoring_service:
                self._monitoring_service.record_request_metrics(
                    duration=duration, status="error", endpoint="execute_sql_query"
                )
            raise
