...
class MonitoringApplicationService(BaseApplicationService):
    ...
    async def perform_comprehensive_health_check(self) -> Dict[str, Any]:
        ...
        # 更新健康狀態
        self._update_health_status("system_overall", overall_status, 
                                 f"整體系統狀態: {overall_status}")
        
        self._system_stats["last_health_check"] = datetime.now()
        
        # 記錄健康檢查指標
        self.record_metric("health_check.duration", 
                          time.time() - start_time, 
                          "seconds")
        
        self.record_metric("health_check.status", 
                          1 if overall_status == "healthy" else 0, 
                          "boolean")
        
        self.logger.info("健康檢查完成", 
                        overall_status=overall_status,
    ...
    def _check_performance_metrics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if response_times:
                ...
                self.record_metric("performance.avg_response_time", 
    ...
    def get_dashboard_data(self) -> Dict[str, Any]:
        ...
        return {
            "timestamp": datetime.now().isoformat(),
            "system_uptime": (datetime.now() - self._system_stats["start_time"]).total_seconds(),
            "total_requests": self._system_stats["total_requests"],
            "error_count": self._system_stats["error_count"],
            "error_rate": (self._system_stats["error_count"] / 
                          max(self._system_stats["total_requests"], 1)) * 100,
            "last_health_check": (
                self._system_stats["last_health_check"].isoformat() 
                if self._system_stats["last_health_check"] else None
            ),
            "recent_metrics_count": len(recent_metrics),
    ...
    def get_performance_report(self, hours: int = 24) -> Dict[str, Any]:
        ...
        # 分析不同類型的指標
        response_times = [m.value for m in relevant_metrics if m.name == "request.duration"]
        error_counts = [m.value for m in relevant_metrics if m.name == "request.count" 
        ...
        report = {
            ...
            "performance_summary": {
                ...
                "total_requests": len([m for m in relevant_metrics if m.name == "request.count"]),
    ...
    async def _perform_health_checks(self) -> Dict[str, Dict[str, Any]]:
        ...
        # 檢查系統統計
        uptime_hours = (datetime.now() - self._system_stats["start_time"]).total_seconds() / 3600
