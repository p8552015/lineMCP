...
class PostgreSQLCommand:
        ...
        try:
            ...
            postgres_context = {
                ...
                "tables": {
                    ...
                    "orders": ["id", "customer_name", "product_id", "quantity", "total_amount", "status"],
                    "machines": ["id", "name", "status", "temperature", "utilization_rate", "location"]
                },
    ...
    async def _execute_postgres_query(self, sql_query: str, original_query: str) -> Dict[str, Any]:
        """執行 PostgreSQL 查詢"""
        try:
            # 獲取 MCP 客戶端管理器
            ...
            if result.get("success"):
                # 處理查詢結果
                return await self._format_postgres_result(result, sql_query, original_query)
            else:
    ...
    async def _format_postgres_result(self, result: Dict[str, Any], sql_query: str, original_query: str) -> Dict[str, Any]:
        """格式化 PostgreSQL 查詢結果"""
        try:
            ...
            # 格式化顯示文本
            display_text = self._format_display_text(formatted_data, original_query, summary)
    ...
    def _format_display_text(self, data: List[Dict], original_query: str, summary: Dict) -> str:
        """格式化顯示文本"""
        if not data:
