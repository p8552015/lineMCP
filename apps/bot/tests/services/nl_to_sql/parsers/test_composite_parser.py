...
class MockParser(IParser):
    ...
    def __init__(self, name: str, confidence: float = 0.8, query_type: QueryType = QueryType.MACHINE_STATUS):
        self.name = name
...
class TestCompositeParser:
    ...
    class TestParsingStrategy:
        ...
        async def test_parse_with_weighted_selection(self, parser, mock_parsers):
            """測試加權選擇策略"""
            # 設置權重影響選擇
            parser.add_parser(mock_parsers[1], 1.5)  # AIParser: 0.7 * 1.5 = 1.05 (有效信心度)
    ...
    class TestErrorHandling:
        ...
        @pytest.mark.asyncio
        async def test_parse_with_capability_assessment_exception(self, parser, mock_parsers):
            """測試能力評估異常"""
            mock_parser = mock_parsers[0]
            mock_parser.set_should_raise(True)
            parser.add_parser(mock_parser)
        ...
        @pytest.mark.asyncio
        async def test_parse_with_all_parsers_low_confidence(self, parser, mock_parsers):
            """測試所有解析器信心度都很低"""
            # 設置高門檻值
            parser.set_fallback_threshold(0.95)
            
            # 添加低信心度解析器
            low_confidence_parser = MockParser("LowConfidence", 0.3, QueryType.MACHINE_STATUS)
    ...
    class TestConfigurationManagement:
        ...
        def test_validate_configuration_invalid_parser(self, parser):
            ...
            assert result["is_valid"] is False
            assert any("未正確實現 IParser 介面" in error 
