...
class TestQueryApplicationService:
    ...
    @pytest.fixture
    def query_service(self, mock_mcp_client_factory, mock_db_service, mock_response_parser):
        """查詢服務實例"""
    ...
    @pytest.mark.asyncio
    async def test_execute_sql_query_success(self, query_service, mock_mcp_client_factory, mock_response_parser):
        """測試成功執行 SQL 查詢"""
        # 設置模擬
        mock_client = await mock_mcp_client_factory()
        mock_client.call_tool.return_value = {"status": "success", "data": "raw_data"}
        mock_response_parser.parse_query_result.return_value = [{"id": 1, "name": "test"}]
    ...
    @pytest.mark.asyncio
    async def test_execute_sql_query_expired_cache(self, query_service, mock_mcp_client_factory, mock_response_parser):
        """測試過期快取的處理"""
        # 設置過期快取
        cache_key = query_service._generate_cache_key("SELECT * FROM test")
        ...
        mock_response_parser.parse_query_result.return_value = [{"id": 1, "name": "fresh"}]
        query_service._initialized = True
    ...
    @pytest.mark.asyncio
    async def test_execute_sql_query_error(self, query_service, mock_mcp_client_factory):
        """測試查詢執行錯誤"""
        # 設置模擬拋出異常
        mock_client = await mock_mcp_client_factory()
    ...
    def test_update_query_stats(self, query_service):
        ...
        assert query_service._query_stats["avg_execution_time"] == 1.0  # (0.5 + 1.5) / 2
    ...
    async def test_get_table_info_specific_table(self, query_service):
        ...
        with patch.object(query_service, 'execute_sql_query') as mock_execute:
            ...
            assert "SELECT COUNT(*) as count FROM machines" in calls[1][0][0]  # 第一個位置參數
    ...
    @pytest.mark.asyncio
    async def test_health_checks_mcp_error(self, query_service, mock_mcp_client_factory):
        """測試 MCP 連接錯誤的健康檢查"""
        # 設置 MCP 客戶端拋出異常
        mock_client = await mock_mcp_client_factory()
