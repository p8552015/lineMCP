...
class TestPerformanceMetric:
    ...
    def test_metric_default_tags(self):
        """測試預設標籤"""
        metric = PerformanceMetric(
            name="test.metric", value=1.0, unit="count", timestamp=datetime.now()
        )
...
class TestHealthStatus:
    ...
    def test_health_status_creation(self):
        """測試健康狀態創建"""
        status = HealthStatus(
            component="database", status="healthy", message="All connections active"
        )
...
class TestMonitoringApplicationService:
    ...
    async def test_service_initialization(self, monitoring_service):
        ...
        startup_metrics = [
            m for m in monitoring_service._metrics if m.name == "service.startup"
        ]
    ...
    def test_record_request_metrics(self, monitoring_service):
        ...
        monitoring_service.record_request_metrics(
            duration=0.5, status="success", endpoint="test_api"
        )
        ...
        # 驗證指標記錄
        duration_metrics = [
            m for m in monitoring_service._metrics if m.name == "request.duration"
        ]
        ...
        # 記錄錯誤請求
        monitoring_service.record_request_metrics(duration=1.0, status="error")
    ...
    def test_record_slow_request(self, monitoring_service):
        ...
        # 記錄慢請求（超過基線）
        monitoring_service.record_request_metrics(
            duration=2.0, status="success"  # 超過基線的 1.0 秒
        )

        # 應該記錄慢請求指標
        slow_metrics = [
            m for m in monitoring_service._metrics if m.name == "request.slow"
        ]
        assert len(slow_metrics) == 1

    @pytest.mark.asyncio
    async def test_comprehensive_health_check(
        self, monitoring_service, mock_service_context
    ):
        """測試全面健康檢查"""
        # 設置服務上下文回應
        ...
        with patch.object(monitoring_service, "_check_system_resources") as mock_system:
            with patch.object(
                monitoring_service, "_check_performance_metrics"
            ) as mock_perf:
    ...
    @pytest.mark.asyncio
    async def test_health_check_with_degraded_service(
        self, monitoring_service, mock_service_context
    ):
        """測試降級服務的健康檢查"""
        # 設置服務上下文回應為降級
    ...
    def test_check_system_resources_healthy(
        self, mock_disk, mock_memory, mock_cpu, monitoring_service
    ):
        """測試系統資源檢查（健康狀態）"""
        # 設置模擬回傳健康的資源使用率
        mock_cpu.return_value = 30.0
        mock_memory.return_value = Mock(
            used=200 * 1024 * 1024, percent=40.0
        )  # 200MB, 40%
    ...
    def test_check_system_resources_degraded(
        ...
        mock_memory.return_value = Mock(
            used=600 * 1024 * 1024, percent=85.0
        )  # 600MB，超過500MB基線
    ...
    def test_check_performance_metrics(self, monitoring_service):
        """測試效能指標檢查"""
        # 設置一些請求指標
        monitoring_service._system_stats = {"total_requests": 100, "error_count": 3}
    ...
    def test_get_performance_report(self, monitoring_service):
        """測試效能報告獲取"""
        # 設置一些測試數據
        monitoring_service._system_stats = {"total_requests": 200, "error_count": 10}

        # 添加響應時間指標
        for i in range(5):
            monitoring_service._metrics.append(
                PerformanceMetric(
                    "request.duration", 0.5 + i * 0.2, "seconds", datetime.now()
                )
            )
        ...
        for i in range(2):
            monitoring_service._metrics.append(
                PerformanceMetric(
                    "request.count", 1, "count", datetime.now(), {"status": "error"}
                )
            )
    ...
    def test_generate_performance_recommendations(self, monitoring_service):
        ...
        recommendations = monitoring_service._generate_performance_recommendations(
            high_response_metrics
        )
...
class TestMonitoringServiceIntegration:
    ...
    async def test_full_monitoring_lifecycle(self):
        ...
        service_context.health_check_all = AsyncMock(
