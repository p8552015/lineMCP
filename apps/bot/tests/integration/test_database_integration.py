...
class TestDatabaseSchema:
    ...
    async def test_machines_table_structure(self, db_connection):
        ...
        for required_col in required_columns:
            assert required_col in column_names, f"machines 表缺少必需欄位: {required_col}"
    ...
    async def test_machine_faults_table_structure(self, db_connection):
        ...
        for required_col in required_columns:
            assert required_col in column_names, f"machine_faults 表缺少必需欄位: {required_col}"
    ...
    async def test_timeout_handling(self, db_connection):
        ...
        try:
            result = await asyncio.wait_for(
                db_connection.fetchval('SELECT pg_sleep(0.1)'),
                timeout=1.0
            )
            # pg_sleep 返回空值
