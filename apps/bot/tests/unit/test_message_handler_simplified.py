...
class TestMessageHandlerCore:
    ...
    async def test_mcp_client_initialization(self):
        ...
        with patch(
            "src.services.message_handler.get_unified_mcp_client"
        ) as mock_get_client:
    ...
    async def test_process_message_command_routing(self):
        ...
        # 模擬所有依賴
        with (
            patch.object(handler, "_handle_command") as mock_handle_command,
    ...
    async def test_command_handler_routing(self):
        ...
        # 模擬各種指令處理方法
        with (
            patch.object(handler, "_handle_sql_command") as mock_sql,
