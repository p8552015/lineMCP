...
class TestMCPConnection:
    ...
    async def test_environment_variables_setup(self):
        """測試環境變數設置"""
        # 檢查關鍵環境變數
        required_vars = ["ASYNCIO_FORCE_SELECT_SELECTOR"]
    ...
    async def test_mock_connection_attempt(self, mcp_client):
        """測試模擬連接嘗試"""
        with patch.object(mcp_client, "_verify_process_health") as mock_health:
    ...
    async def test_error_handling_keyerror_fix(self, mcp_client):
        ...
        # 測試查詢請求（這應該觸發我們的修復邏輯）
        query_data = {"query": "SELECT 1", "server_name": server_name}
