...
class TestServiceRegistryIssues:
    ...
    def test_enhanced_factory_initialization(self, enhanced_factory):
        ...
        for method_name in basic_methods:
            assert hasattr(
                enhanced_factory, method_name
            ), f"缺少基本方法: {method_name}"
    ...
    def test_enhanced_mcp_client_registration(self, enhanced_factory):
        ...
        try:
            ...
            if hasattr(enhanced_factory, "get_enhanced_mcp_client"):
                ...
                # 檢查客戶端方法
                required_methods = ["connect_to_server", "call_tool", "get_system_info"]

                for method_name in required_methods:
                    assert hasattr(
                        client, method_name
                    ), f"客戶端缺少方法: {method_name}"

                print("✅ 增強 MCP 客戶端註冊正常")
            else:
                print("⚠️ 增強 MCP 客戶端方法不存在，使用回退機制")
    ...
    def test_enhanced_mcp_config_registration(self, enhanced_factory):
        ...
        try:
            ...
            if hasattr(enhanced_factory, "get_enhanced_mcp_config"):
                ...
                # 檢查配置方法
                required_methods = ["get_base_config", "analyze_runtime_environments"]
...
class TestCompatibilityInterfaces:
    ...
    async def test_enhanced_mcp_client_compatibility(self):
        ...
        try:
            ...
            # 測試不同參數組合的兼容性
            client_nodecomman = get_enhanced_mcp_client(
                use_nodecomman=True, fallback_enabled=True
            )
            client_fallback = get_enhanced_mcp_client(
                use_nodecomman=False, fallback_enabled=True
            )
...
class TestCriticalIssueDetection:
    ...
    def test_detect_service_registry_parameter_issues(self):
        ...
        try:
            ...
            # 檢查關鍵方法的參數
            critical_methods = ["get_mcp_config", "get_mcp_connection_pool"]
            ...
            for method_name in critical_methods:
                if hasattr(factory, method_name):
                    ...
                    if required_params:
                        issues.append(
                            f"方法 {method_name} 有意外的必需參數: {[p.name for p in required_params]}"
                        )
                else:
        ...
        try:
            ...
            try:
                ...
                for method_name in critical_methods:
                    ...
                    # 如果兩者都有該方法，檢查簽名兼容性
                    if hasattr(production_client, method_name) and hasattr(
                        enhanced_client, method_name
                    ):
                        import inspect

                        prod_sig = inspect.signature(
                            getattr(production_client, method_name)
                        )
                        enh_sig = inspect.signature(
                            getattr(enhanced_client, method_name)
                        )
                        ...
                        if prod_params != enh_params:
                            issues.append(
                                f"方法 {method_name} 參數數量不一致: 生產={prod_params}, 增強={enh_params}"
                            )
