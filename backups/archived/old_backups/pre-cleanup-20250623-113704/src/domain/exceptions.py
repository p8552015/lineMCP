...
class DatabaseQueryException(BotException):
    ...
    def __init__(self, query: str, reason: str, query_type: Optional[str] = None):
...
class BusinessLogicException(BotException):
    ...
    def __init__(self, operation: str, reason: str, user_message: Optional[str] = None):
...
class ExternalServiceException(BotException):
    ...
    def __init__(self, service: str, operation: str, status_code: Optional[int] = None, reason: str = ""):
        ...
        super().__init__(
            message=message,
            user_message=user_message,
            details={"service": service, "operation": operation, "status_code": status_code, "reason": reason},
...
def create_db_error(query: str, reason: str, query_type: Optional[str] = None) -> DatabaseQueryException:
    """創建資料庫錯誤"""
    return DatabaseQueryException(query, reason, query_type)


def create_mcp_error(server: str, operation: str, reason: str) -> MCPConnectionException:
    """創建 MCP 連接錯誤"""
