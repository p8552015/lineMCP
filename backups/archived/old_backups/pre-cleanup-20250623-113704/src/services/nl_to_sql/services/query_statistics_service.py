...
class QueryStatisticsService(IStatistics):
    ...
    def __init__(self, max_history_size: int = 10000):
        ...
        self._success_counts: defaultdict = defaultdict(int)
        self._failure_counts: defaultdict = defaultdict(int)
        self._confidence_scores: defaultdict = defaultdict(list)
        self._parse_times: defaultdict = defaultdict(list)
        self._history: List[Dict[str, Any]] = []
        self._error_types: defaultdict = defaultdict(int)
        self._query_type_stats: defaultdict = defaultdict(int)
        
        # 🔥 關鍵修復：啟動時進行數據完整性檢查和清理
        self._perform_startup_integrity_check()
        
        logger.info("📊 查詢統計服務初始化完成", max_history_size=max_history_size)
    
    def _perform_startup_integrity_check(self) -> None:
        """
        執行啟動時的數據完整性檢查和清理
        ...
        """
        try:
            logger.info("🔍 執行啟動時數據完整性檢查...")
            
            # 檢查並清理 confidence 數據
            cleaned_confidence_count = 0
            for parser_type, confidences in list(self._confidence_scores.items()):
                original_count = len(confidences)
                # 使用類型安全轉換，只保留有效的數值
                safe_confidences = self._safe_numeric_list_conversion(confidences)
                self._confidence_scores[parser_type] = safe_confidences
                
                cleaned_count = original_count - len(safe_confidences)
                if cleaned_count > 0:
                    cleaned_confidence_count += cleaned_count
                    logger.warning(f"清理了 {parser_type} 的 {cleaned_count} 個無效 confidence 值")
            
            # 檢查並清理 parse_times 數據
            cleaned_times_count = 0
            for parser_type, times in list(self._parse_times.items()):
                original_count = len(times)
                # 使用類型安全轉換，只保留有效的數值
                safe_times = self._safe_numeric_list_conversion(times)
                self._parse_times[parser_type] = safe_times
                
                cleaned_count = original_count - len(safe_times)
                if cleaned_count > 0:
                    cleaned_times_count += cleaned_count
                    logger.warning(f"清理了 {parser_type} 的 {cleaned_count} 個無效 parse_time 值")
            
            # 檢查歷史記錄中的數據完整性
            cleaned_history_count = 0
            valid_history = []
            for record in self._history:
                try:
                    # 驗證歷史記錄的基本結構
                    if isinstance(record, dict) and 'timestamp' in record:
                        # 如果有 metadata 包含 confidence，確保它是數值類型
                        if 'metadata' in record and isinstance(record['metadata'], dict):
                            if 'confidence' in record['metadata']:
                                original_confidence = record['metadata']['confidence']
                                safe_confidence = self._safe_confidence_conversion(original_confidence, 0.0)
                                record['metadata']['confidence'] = safe_confidence
                                
                                if str(original_confidence) != str(safe_confidence):
                                    logger.debug(f"修正歷史記錄中的 confidence: {original_confidence} -> {safe_confidence}")
                        
                        valid_history.append(record)
                    else:
                        cleaned_history_count += 1
                except Exception as e:
                    logger.warning(f"清理無效歷史記錄: {e}")
                    cleaned_history_count += 1
            
            self._history = valid_history
            
            # 記錄清理結果
            total_cleaned = cleaned_confidence_count + cleaned_times_count + cleaned_history_count
            if total_cleaned > 0:
                logger.info("🧹 數據完整性檢查完成", 
                           cleaned_confidence=cleaned_confidence_count,
                           cleaned_times=cleaned_times_count, 
                           cleaned_history=cleaned_history_count,
                           total_cleaned=total_cleaned)
            else:
                logger.info("✅ 數據完整性檢查完成，未發現需要清理的數據")
            
        except Exception as e:
            logger.error("❌ 啟動時數據完整性檢查失敗", error=str(e))
            # 如果檢查失敗，為了安全起見，清空所有數據
            logger.warning("為了確保類型安全，將清空所有統計數據")
    ...
    def get_recent_activity(self, hours: int = 24) -> List[Dict[str, Any]]:
        ...
        for record in self._history:
            ...
            if isinstance(record_timestamp, str):
                try:
                    ...
                    logger.warning(
                        ...
                        original_type=type(record["timestamp"]).__name__
                    )
                    continue
            elif not isinstance(record_timestamp, (int, float)):
                # 不是數值類型，跳過
                logger.warning(
                    f"時間戳類型錯誤",
                    timestamp_type=type(record_timestamp).__name__,
                    timestamp_value=record_timestamp
                )
                continue
            ...
            if record_timestamp >= cutoff_time:
                ...
                record_copy["timestamp"] = float(record_timestamp)
                recent_records.append(record_copy)
