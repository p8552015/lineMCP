...
class ModelsCommandHandler(CommandHandler):
        ...
        try:
            ...
            for model in models_info['models']:
                if (model.get('name', '').lower() == model_name.lower() or 
                    model.get('id', '').lower() == model_name.lower()):
                    target_model = model
                    break
            
            if not target_model:
                available_models = [m.get('name', 'unknown') for m in models_info['models']]
                return TextMessage(
            ...
            return self._format_model_details(target_model, models_info['current_model'])
    ...
    def _format_models_list(self, models_info: dict) -> TextMessage:
        ...
        for provider, models in providers.items():
            ...
            for model in models[:5]:  # 每個提供商最多顯示5個模型
                ...
                is_current = (current_model_name and 
    ...
    def _format_model_details(self, model: dict, current_model: dict) -> TextMessage:
        ...
        is_current = (current_model and 
                     current_model.get('name') == model_name)
