...
class AIEnhancedParser(IParser):
    ...
    async def parse(
        ...
        try:
            ...
            enhanced_result, confidence = await self._ai_service.enhance_natural_language_query(
                text, database_schema
            )
            
            # 解析 AI 返回的結果
            logger.debug("🔍 AI 返回結果", ai_result=enhanced_result[:200], confidence=confidence)
            parsed_query = self._parse_ai_result(enhanced_result, text, confidence)
            
            logger.debug("✅ AI 解析完成", 
...
"""
    ...
    def _prepare_ai_context(
        ...
        ai_context = {
            ...
            "supported_entities": [
                "machine_id", "department", "time_range", 
                "fault_type", "production_metric"
            ]
        }
    ...
    def _parse_ai_result(
        ...
        Returns:
            ParsedQuery: 解析結果
        """
        # 🔥 關鍵修復：確保 ai_confidence 為數值類型
        try:
            if isinstance(ai_confidence, str):
                if ai_confidence.strip() == '':
                    ai_confidence = 0.0
                else:
                    ai_confidence = float(ai_confidence.strip())
            ...
            # 轉換失敗時使用預設值
            logger.warning("⚠️ AI confidence 類型轉換失敗，使用預設值", 
        ...
        try:
            ...
            if not isinstance(ai_result, str):
                ...
                return self._create_error_query(original_text, f"AI 結果類型錯誤: {type(ai_result)}")
            
            ai_result = ai_result.strip()
            if not ai_result:
                logger.warning("⚠️ AI 返回空結果")
                return self._parse_text_ai_result(original_text, original_text, ai_confidence)
            ...
            if ai_result.startswith("{") and ai_result.endswith("}"):
                try:
                    ...
                    if isinstance(result_data, dict):
                        return self._create_query_from_ai_data(result_data, original_text, ai_confidence)
                    else:
                        logger.warning("⚠️ AI JSON 結果不是字典格式", result_type=type(result_data))
                        return self._parse_text_ai_result(ai_result, original_text, ai_confidence)
                except json.JSONDecodeError as e:
                    logger.warning("⚠️ AI 結果 JSON 解析失敗", error=str(e), ai_result=ai_result[:100])
                    return self._parse_text_ai_result(ai_result, original_text, ai_confidence)
            else:
                # 處理非 JSON 格式的 AI 回應
                return self._parse_text_ai_result(ai_result, original_text, ai_confidence)
            ...
            logger.error("❌ AI 結果解析異常", error=str(e), ai_result_preview=str(ai_result)[:100] if ai_result else "None")
    ...
    def _create_query_from_ai_data(
        ...
        # 🔥 關鍵修復：確保 confidence 為數值類型
        confidence_value = ai_data.get("confidence", ai_confidence)
        try:
            if isinstance(confidence_value, str):
                if confidence_value.strip() == '':
                    confidence = 0.0
                else:
                    confidence = float(confidence_value.strip())
            elif confidence_value is None:
                confidence = ai_confidence  # 使用已經驗證過的 ai_confidence
            else:
            ...
            # 轉換失敗時使用 ai_confidence
            logger.warning("⚠️ AI 數據中的 confidence 轉換失敗，使用預設值", 
        ...
        if query_type == QueryType.UNKNOWN:
            query_type = self._infer_query_type_from_content(original_text, target_entities, parameters)
    ...
    def _parse_text_ai_result(
        ...
        try:
            ...
            # 轉換失敗時使用預設值
            logger.warning("⚠️ 文字解析 AI confidence 類型轉換失敗，使用預設值", 
                         original_confidence=ai_confidence)
            ai_confidence = 0.0
        ...
        if query_type == QueryType.UNKNOWN:
            logger.warning("⚠️ 無法從 AI 結果推斷查詢類型，使用預設", 
                         ai_result=ai_result[:100], original_text=original_text)
        ...
        return ParsedQuery(
            ...
            sql_query="",  # SQL 將由 QueryBuilder 建構
            parameters=parameters,
            confidence=ai_confidence * 0.8,  # 稍微降低信心度 - 現在類型安全
    ...
    def _infer_query_type_from_content(
        ...
        # 故障相關關鍵詞
        if any(keyword in text_lower for keyword in ['故障', '問題', '錯誤', '異常', '維修', '近期']):
        ...
        # 部門查詢關鍵詞
        if any(keyword in text_lower for keyword in ['部門', '加工', '組裝', '品管', '維修']):
        ...
        if machine_pattern.search(text) or any('M' in entity.upper() for entity in target_entities):
    ...
    def _infer_query_type_from_sql(self, sql: str) -> QueryType:
        ...
        # 特定機台查詢（有機台 ID 條件）
        if 'machine_id' in sql_lower and ('where' in sql_lower or 'machine_id =' in sql_lower):
            return QueryType.SPECIFIC_MACHINE
    ...
    def _extract_parameters_from_text(self, original_text: str, ai_result: str) -> dict:
        ...
        # 從 AI SQL 結果中提取機台 ID
        sql_machine_pattern = re.compile(r"machine_id\s*=\s*['\"]([^'\"]+)['\"]", re.IGNORECASE)
    ...
    def _create_error_query(self, original_text: str, error_message: str) -> ParsedQuery:
        """
        創建錯誤查詢結果
