...
class MessagingApplicationService(BaseApplicationService):
    ...
    async def process_message(self, 
        ...
        try:
            # 創建帶上下文的 logger
            context_logger = self.logger.bind(user_id=user_id, message_length=len(message_text))
            ...
            if message_type == "command":
                ...
                result = await self._process_natural_language_message(user_id, message_text)
    ...
    async def _process_command_message(self, user_id: str, message_text: str) -> Message:
        """
        處理指令訊息
    ...
    async def _process_natural_language_message(self, user_id: str, message_text: str) -> Message:
        """
        處理自然語言訊息
        ...
        try:
            # 使用自然語言服務處理
            result = await self.nl_service.parse_natural_language(message_text)
            
            # 格式化結果
            if result.get("success", False):
                return self.message_formatter.format_query_result(
    ...
    def _update_session_result(self, 
                             user_id: str, 
