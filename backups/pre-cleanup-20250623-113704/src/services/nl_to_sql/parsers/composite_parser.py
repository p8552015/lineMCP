...
class CompositeParser(IParser):
    ...
    async def _evaluate_parser_capabilities(
        ...
        for parser in self._parsers:
            try:
                ...
                logger.debug("📊 解析器能力評估",
                           parser=parser.__class__.__name__,
                           confidence=confidence)
                
            except Exception as e:
                logger.warning("⚠️ 解析器能力評估異常",
                             parser=parser.__class__.__name__,
                             error=str(e))
    ...
    def _select_best_parser(
        self, 
        capabilities: List[Tuple[IParser, float]]
    ) -> Tuple[Optional[IParser], float]:
        """
        選擇最佳解析器
