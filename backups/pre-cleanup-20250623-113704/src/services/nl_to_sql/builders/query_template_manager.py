...
class QueryTemplateManager(ITemplateManager):
    ...
    def get_template(self, query_type: QueryType) -> str:
        ...
        logger.debug("📋 獲取模板", query_type=query_type.value, template_length=len(template))
    ...
    def _validate_sql_security(self, template_upper: str) -> bool:
        ...
        try:
            ...
            forbidden_keywords = security_config.get('forbidden_keywords', 
                ["DROP", "DELETE", "UPDATE", "INSERT", "ALTER", "TRUNCATE", "EXEC", "UNION", "SCRIPT"])
            ...
            for keyword in forbidden_keywords:
                ...
                if re.search(pattern, template_upper, re.MULTILINE):
                    logger.warning("⚠️ 模板包含危險的 SQL 語句", 
                                 keyword=keyword, 
    ...
    def _validate_mcp_compatibility(self, template_upper: str) -> bool:
        ...
        for pattern in potentially_problematic_patterns:
            if re.search(pattern, template_upper, re.IGNORECASE):
                logger.warning("⚠️ 模板包含可能被 MCP 服務器限制的構造", 
                             pattern=pattern, 
    ...
    def _is_sql_properly_formatted(self, template_upper: str) -> bool:
        ...
        if open_parens != close_parens:
            logger.warning("⚠️ SQL 括號不匹配", 
                         open_count=open_parens, 
                         close_count=close_parens)
            return False
    ...
    def _load_default_templates(self) -> None:
        ...
        for query_type, template in default_templates.items():
            try:
                ...
                logger.error("❌ 預設模板載入失敗", 
                           query_type=query_type.value, 
                           error=str(e))
        
        logger.info("📋 預設模板載入完成", 
    ...
    def _update_template_metadata(self, query_type: QueryType, template: str) -> None:
        ...
        complexity_indicators = {
            ...
            'aggregations': len(re.findall(r'(AVG|SUM|COUNT|MAX|MIN)\s*\(', template, re.IGNORECASE)),
        ...
        self._template_metadata[query_type] = {
            ...
            'estimated_performance': self._estimate_template_performance(complexity_score),
        }
    ...
    def get_template_stats(self) -> Dict[str, Any]:
        ...
        total_parameters = sum(
            len(self.get_template_parameters(qt)) 
        ...
        for query_type in self._templates.keys():
            ...
            complexity_distribution[performance] = complexity_distribution.get(performance, 0) + 1
        ...
        return {
            ...
            'average_complexity': sum(
                ...
                for qt in self._templates.keys()
            ) / total_templates if total_templates > 0 else 0,
        }
        ...
        for query_type, template in self._templates.items():
            try:
                ...
                if mcp_validation['is_mcp_compatible']:
                    validation_results['mcp_compatible_templates'].append(query_type.value)
                else:
                    validation_results['mcp_incompatible_templates'].append(query_type.value)
                    validation_results['warnings'].extend([
                        f"{query_type.value}: {error}" for error in mcp_validation['errors']
                    ])
                
                # 收集建議
                if mcp_validation['recommendations']:
                    validation_results['warnings'].extend([
                        f"{query_type.value}: {rec}" for rec in mcp_validation['recommendations']
                    ])
