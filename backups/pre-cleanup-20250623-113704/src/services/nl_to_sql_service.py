...
class NaturalLanguageToSQLService:
    ...
    async def parse_natural_language(
        ...
        try:
            ...
            # 記錄成功統計 - 🔥 關鍵修復：確保 confidence 為數值類型
            safe_confidence = result.confidence
            try:
                # 確保 confidence 是數值類型，防止統計服務中的類型比較錯誤
                if isinstance(safe_confidence, str):
                    if safe_confidence.strip() == '':
                        safe_confidence = 0.0
                    else:
                        safe_confidence = float(safe_confidence.strip())
                elif safe_confidence is None:
                    safe_confidence = 0.0
                else:
                ...
                # 轉換失敗時使用預設值
                safe_confidence = 0.0
                logger.warning("統計記錄時 confidence 類型轉換失敗", 
                             original_confidence=str(result.confidence),
                             original_type=type(result.confidence).__name__)
